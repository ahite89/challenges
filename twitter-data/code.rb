...
twitter_data.each do |hash|
  hash.each {|user, describe| puts "#{user}: #{describe["description"]}" if describe["description"] != nil }
  hash.each {|user, describe| puts "#{user}: N/A" if describe["description"] == nil}
end
...
follow = {}
twitter_data.each do |hash|
  hash.each {|user, describe| follow[user] = describe["number of followers"]}
end

follow.each {|user, followers| puts "#{user}: #{followers}"}

#username, length of tweet

tweet = {}
twitter_data.each do |hash|
  hash.each {|user, describe| tweet[user] = describe["latest tweet"]}
end

tweet.each {|user, tweet| puts "#{user}/s latest tweet was #{tweet.length} characters long"}

#username, last twenty tweets

twenty = {}
twitter_data.each do |hash|
  hash.each {|user, describe| twenty[user] = describe["last twenty tweets"]}
end

twenty.each {|user, twenty| puts "#{user} used #{twenty.inject(:+).length} characters in their last twenty tweets"}

#user with most followers

puts follow.select {|user, followers| followers == follow.values.max}

#user with most friends

friends = {}
twitter_data.each do |hash|
  hash.each {|user, describe| friends[user] = describe["number of friends"]}
end

puts friends.select {|user, friend| friend == friends.values.max}

#user with most tweets

number_tweets = {}
twitter_data.each do |hash|
  hash.each {|user, describe| number_tweets[user] = describe["number of tweets"]}
end

puts number_tweets.select {|user, tweet| tweet == number_tweets.values.max}

#users with description

twitter_data.each do |hash|
  hash.each {|user, describe| puts user if describe["description"] != nil}
end

#users with location

twitter_data.each do |hash|
  hash.each {|user, describe| puts user if describe["location"] != nil}
end
